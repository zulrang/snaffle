...
### Functional
- Carry a change through phases: spec → plan → implement → validate → commit/PR, with a spike inserted only to retire an open question — never as a fixed phase, and orthogonal to regime (D25) — the minimal (two-way) regime collapsing spec+plan into an inline target and reusing existing tests as the oracle when they cover the change (D25).
...
Touches money, auth, persisted schema, a public contract, or an irreversible migration → one-way door → full regime + mandatory human sign-off; this is the core trigger set, not an exhaustive one — other decisions add their own one-way doors (e.g. modifying a frozen oracle, D7). Otherwise → two-way door → minimal regime (D25), auto-merge on green (subject to the risk-weighted human sample, D11). *Why:* process should track reversibility, not be uniform; this prevents heavyweight ceremony from taxing trivial work. *Rejected:* one uniform process for all changes.
...
Pre-merge acceptance is a single deterministic gate, ordered cheapest-first (format/lint → types → affected tests → contract-diff → scope and oracle integrity → spec-traceability → done_when → smoke budgets), where spec-traceability asserts every change in the diff is attributable to a declared requirement (no untraceable edits) and done_when asserts the requirement's frozen completion criteria are satisfied — two distinct stages, not one; it is authoritative, the only thing that can block, and it judges against the frozen acceptance target (D20). Closure is a distinct, positive, lineage-scoped gate, not "queue empty" (D20). Post-launch: the real metric, measured behind a flag over real traffic, with an automated guardrail that auto-rolls-back the flag on threshold breach; a human owns the ramp. *Why:* mechanical defects are caught for free and deterministically, keeping a single source of acceptance truth; intent that a deterministic oracle cannot express is caught by the mandatory human on one-way doors (D5/D11) and by the post-launch metric, not by a second stochastic voice in the merge path. *Rejected:* an LLM that owns or shares the merge verdict; equating queue-drain with completion. (An additive stochastic grader was considered and deferred — see D24.)

**D9 — Stateful and public-contract changes emit expand/contract by construction.**
Any change the door classifier marks as touching persisted state or a public contract is forced to follow an expand/contract choreography rather than a one-shot edit, so every intermediate step is independently safe and reversible. For persisted state the choreography is add-backward-compatible → dual-write/read → backfill → flip → contract; for a public contract it is the versioned analog add-new-version → dual-serve → migrate-callers → remove-old, where the final removal is itself a separate one-way change with its own gate. *Why:* once users create state under a new path — or once external callers depend on a contract shape — naive rollback is impossible, because reverting cannot un-orphan written data or un-break callers that already adapted. *Rejected:* "rollback" as a single revert; shipping a breaking contract reshape in one step.
...
Humans own: door overrides, spec/cut-line approval for one-way doors, spike resolutions, the production ramp, and a risk-weighted *sample* of two-way-door diffs. Everything else is machine-gated. An async human gate suspends the lineage into a first-class parked state carrying everything needed to resume *and* everything needed to decide: the frozen execution plan (D21), declared scope, the proposed diff persisted as a content-addressed artifact (D10/D18), and the evidence that makes the decision legible — the door-classification rationale (why this is a one-way door) and the contract-diff result — so a breaking public-contract or stateful change is explicit to the reviewer rather than inferred from a deliberately benign expand/contract step (D9), and so any authorized process can continue independent of the original ephemeral worktree. Approval is authorization, not completion: it records the decision, binds that approval to the approved plan/diff hash, and transitions only to the intermediate `approved_for_merge` state. A separate continuation step (`resume`, or a re-run on the lineage) takes the single-writer lock (D23), checks current base, reapplies the diff, reruns the POST gate against the current base, commits and pushes idempotently, and only then derives the terminal state: `merged`, or `merge_failed` (covering a base conflict or a red re-gate) with a re-park. An approval whose bound plan/diff hash no longer matches is stale and must re-park for re-approval. *Why:* converts review from O(diffs) to O(decisions), keeps approval usable from any surface, and makes every parked lineage a suspendable, resumable workflow without letting a human click assert completion. *Rejected:* line-by-line human review of all agent output; approval that computes `merged`; baking apply/commit/push into `decisions approve`; requiring a daemon for continuation before the manual dogfood trigger proves the workflow.
...
Project config in TOML. Pipeline state, the HITL queue, and provenance in SQLite, with parked diff artifacts on the filesystem addressed by content hash (no daemon; queryable; OSS-friendly). HITL surface is GitHub PR checks and review comments in CI, with a local CLI/TUI for dev, behind a pluggable adapter. Model tiers are provider-neutral through Pi's `pi-ai` API — each tier (light/mid/heavy) maps to a configured model from any provider (Anthropic, OpenAI, Google, Bedrock, self-hosted), with no hardcoded vendor default. VCS/CI is GitHub-first through a `gh`/Actions adapter. *Why:* sensible zero-dependency defaults for an open-source tool, every one of them swappable via config so no choice is load-bearing.

**D19 — Results are evidence; the control plane derives every state transition.**
An agent result, even a successful one, never mutates authoritative state directly; the orchestrator inspects the validated result and applies the consequent transition itself. A green check does not move the item; an "approved" verdict authorizes continuation but does not close anything or compute `merged` — the merge completion does. *Why:* D6 stops content from widening *scope*; this stops a well-formed-but-wrong result or a human authorization from being trusted as a *transition*. Containment of authority and containment of state-change are separate guarantees, and both must be structural. *Rejected:* agents emitting state transitions directly; approval-as-merge state assertions.
...
Exactly one orchestrator owns a given workspace, enforced by an ownership lock; a second instance fails fast, read-only observers may attach without taking the lock, and "running" is a verifiable runtime claim rather than durable state. Human approval never takes this lock; the separate continuation step does, because it owns worktree-bearing operations like reapply, POST gate, commit, and push. *Why:* a long-running spine that two processes can mutate concurrently corrupts the very state that makes the pipeline auditable, while coupling human approval to a writer lock would make approval location-dependent. *Rejected:* leaving orchestrator concurrency undefined; doing merge work inside the approval action.
...
- **Orchestrator (spine, deterministic):** phase state machine; invokes each agent through Pi's SDK/RPC (scoped context in, structured result out) rather than via harness-side delegation; runs the door and failure classifiers; enforces gates; issues capability grants as per-invocation tool/path scope; owns the HITL queue, parked lineage artifacts, and the provenance log; compiles and freezes the execution plan and refuses stale plans (D21); derives every state transition from validated results and continuation evidence (D19); holds the single-writer ownership lock for worktree-bearing operations (D23); enforces the budget circuit breaker (D22); schedules up to N parallel lineages and back-pressures conflicting ones by declared-scope overlap (D20).
- **`lib/` (deterministic scripts, no LLM, unit-tested, single copy):** door classifier, failure classifier, router, gate runner (typecheck/test/lint/contract-diff/spec-traceability/done_when/perf-smoke), scope guard, oracle freeze, expand/contract emitter, context assembler (retrieval; produces a stable, ordered prompt prefix with a breakpoint and a provider-neutral cache hint, volatile data excluded from the prefix), commit scaffolder, provenance writer, parked-change artifact writer/verifier, baseline/characterization capture, plan compiler/freezer, acceptance-target snapshotter, result applier, budget governor, ownership lock, oracle-escape logger, lineage scheduler / declared-scope conflict detector.
...
The steps below are the shared integrity floor and run in both regimes. The full (one-way) regime additionally runs the spec/plan and dedicated oracle-authoring phases before step 1; the minimal (two-way) regime enters with an inline target and a reused oracle (D25). Human gates differ by policy: one-way doors always pause before merge; two-way doors usually continue but may pause when sampled by D11.
...
5. **Post-gate (deterministic, authoritative):** the same deterministic gate runs independently, including two distinct stages — `spec-traceability` (every change in the diff is attributable to a declared requirement) and `done_when` (the requirement's frozen completion criteria are satisfied); scope and oracle integrity verified. Only this tier can block.
...
7. **Route** (D19): on the strength of the gate verdict (including its `spec-traceability` and `done_when` stages from step 5 — not a second, independent verification), derive the next transition in the control plane, and check budget between steps (D22). A green POST gate routes by door policy and sampling to one of two paths: **auto-merge** (two-way, unsampled) runs the Continuation operation (step 11) inline under the ownership lock already held since step 1; **park for a human gate** (one-way doors, or sampled two-way diffs) enters steps 8–10.
8. **Park for HITL when required:** persist the frozen plan, declared scope, and proposed diff as a content-addressed parked-change artifact; enqueue a decision item bound to that artifact hash and carrying the decision evidence (door-classification rationale and contract-diff result) so the breaking nature of a contract/stateful change is explicit; transition to `awaiting_human`. The original isolated worktree is disposable.
9. **Approve or reject:** approval records authorization and the approved artifact hash, transitioning only to `approved_for_merge`; rejection transitions to `rejected`. Approval does not apply, commit, push, or assert `merged`.
10. **Resume:** a separate lock-taking re-run verifies the approved hash still matches the parked artifact — a stale binding re-parks for re-approval (D11/D21) — and then runs the Continuation operation (step 11) under that freshly taken lock.
11. **Continuation (shared merge operation, both paths):** check the current base, reapply the diff, rerun the POST gate against the current base (because concurrent lineages may have moved the base, D20), commit and push idempotently, and only then derive `merged`. A conflict or a red re-gate transitions to `merge_failed` and re-parks when operator input is needed. The auto-merge branch of step 7 runs this inline under the lock held since step 1; the resume path (step 10) runs it under a freshly taken single-writer lock (D23). Human approval never takes the lock; this step always does the worktree-bearing work.
12. **Closure** is a positive, lineage-scoped decision, not queue-emptiness (D20).
