...
Humans own: door overrides, spec/cut-line approval for one-way doors, spike resolutions, the production ramp, and a risk-weighted *sample* of two-way-door diffs. Everything else is machine-gated. An async human gate suspends the lineage into a first-class parked state carrying everything needed to resume: the frozen execution plan (D21), declared scope, and the proposed diff persisted as a content-addressed artifact (D10/D18), so any authorized process can continue independent of the original ephemeral worktree. Approval is authorization, not completion: it records the decision, binds that approval to the approved plan/diff hash, and transitions only to an intermediate approved/queued-for-merge state. A separate continuation step (`resume`, or a re-run on the lineage) takes the single-writer lock (D23), checks current base, reapplies the diff, reruns the POST gate against the current base, commits and pushes idempotently, and only then derives the terminal state: `merged`, or `conflict`/`merge_failed` with a re-park. An approval whose bound plan/diff hash no longer matches is stale and must re-park for re-approval. *Why:* converts review from O(diffs) to O(decisions), keeps approval usable from any surface, and makes every parked lineage a suspendable, resumable workflow without letting a human click assert completion. *Rejected:* line-by-line human review of all agent output; approval that computes `merged`; baking apply/commit/push into `decisions approve`; requiring a daemon for continuation before the manual dogfood trigger proves the workflow.
...
7. **Verify against the frozen spec** (`done_when` assertion) and **derive the next transition in the control plane** (D19). A green POST gate may either continue toward merge or park for a human gate, depending on door policy and sampling. Budget is checked between steps (D22).
8. **Park for HITL when required:** persist the frozen plan, declared scope, and proposed diff as a content-addressed parked-change artifact; enqueue a decision item bound to that artifact hash; transition to `awaiting_human`. The original isolated worktree is disposable.
...
10. **Resume continuation:** a separate resume/re-run takes the single-writer lock, checks the current base, verifies the approved hash still matches the parked artifact, reapplies the diff, reruns POST gate against the current base, commits and pushes idempotently, and only then derives `merged`; conflict, stale approval, or red POST gate transitions to `merge_failed` and re-parks when operator input is needed.
11. **Closure** is a positive, lineage-scoped decision, not queue-emptiness (D20).
